...
template <typename T> void check_result(T val, char const *str) {
  if (*str == 'n')
    return;
...
template <typename T> inline void read_array(reader_t &rd, T &val) {
  if (rd.expect('[') == false) {
...
template <typename U, typename T> inline void assign(reader_t &rd, T &t) {
...
template <typename T>
inline std::enable_if_t<is_reflection_v<T>, bool>
from_json(T &&t, const char *buf, size_t len = -1) {
  g_has_error = false;
  reader_t rd(buf, len);
  do_read(rd, t);
