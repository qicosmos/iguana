...
- protobuf 自定义字段号支持 C++26 值注解，减少 `YLT_REFL_PB` 依赖。
- 非 C++26 路径补齐 `pb_members` helper，能表达 C++26 注解路径已经具备的 proto3 wire-only schema metadata。
- 公开兼容 API 尽量保留，例如 `visit_members` 仍然支持 `visitor(fields...)` 这种调用形态。
...
| `member_count.hpp`、`member_names.hpp`、`member_ptr.hpp`、`member_value.hpp`、`user_reflect_macro.hpp` | C++26 下接入 `std::meta` 字段来源，`YLT_REFL`/`YLT_REFL_PRIVATE` 保留为兼容入口。 | 保持公开 API 和旧用户代码可编译，同时让 C++26 路径不再依赖宏生成字段。 | 旧路径是否完全保留；C++26 下 no-op 宏是否改变私有反射预期。 |
...
| 保留公开兼容 API | 已完成 | `visit_members`、`object_to_tuple`、`struct_to_tuple` 保持原接口；内部字段来源换成 C++26 反射。 |
...
| `template for` 替代旧式逐字段展开 | 已完成 | 逐字段执行场景已改为 `template for`；tuple/frozen map/variant 等需要参数包生成的场景保留 pack expansion。 |
...
- 先构建编译期 `frozen::unordered_map`，把字段名映射到字段 index。
- 再用 `template for` 按 index 分发到 `obj.[:member:]`。
- 这样避免了原来的字段名线性扫描，也避免了旧实现的 offset 指针算术。
...
- `object_to_tuple(t)` 使用 `std::tie(t.[:member:]...)`。
- `struct_to_tuple<T>()` 返回字段类型指针 tuple，保留旧 API 形态。
- `visit_members(t, visitor)` 仍保留，因为它是公开兼容 API，用户代码依赖 `visitor(field0, field1, ...)` 参数包。

这里没有强行把所有 `index_sequence` 改成 `template for`。原因是 tuple 构造、tuple_cat、frozen map 初始化需要参数包展开，`template for` 更适合逐字段执行逻辑，不适合直接替代所有 pack 生成场景。
...
## 为什么还保留 visit_members

`visit_members` 不是旧实现细节，而是公开 API：
...
C++26 的 `template for` 能简化逐字段访问，但不能直接替代“把全部字段一次性作为参数包传给用户 visitor”的契约。因此 C++26 分支内部仍使用 pack expansion 实现 `visit_members`，但字段来源已经是 `std::meta`，不再是宏生成的结构化绑定。
...
仍保留 `index_sequence` 的主要场景：

- tuple 构造：`std::tie(field...)`、字段类型 tuple、`std::tuple_cat`。
- frozen map 初始化：需要 `{name, index}...` 这种参数包。
...
- `reflect26_dispatch.hpp` 的 map lookup + runtime index dispatch 是否满足性能预期。
